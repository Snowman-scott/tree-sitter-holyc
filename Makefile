...
HOMEPAGE_URL := https://github.com/snowman-scott/tree-sitter-holyc.git
VERSION := 0.1.0
DESCRIPTION := Tree-sitter grammar for the HolyC programming language (from TempleOS, made by Terry A. Davis)
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/holyc '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/holyc
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/holyc
